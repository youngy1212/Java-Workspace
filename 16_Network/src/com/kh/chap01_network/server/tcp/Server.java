...
public class Server {
	 ...
	 * 6. 클라이언트 정보를 통해서 inputStram 생성
	 * 7. 스트림을 통해 입력 및 출력
	 * 8. 통신종료
	 * 	- inputStram, OutputStram, Socket을 전부 닫아줘야함(close())
	 */
		...
		try {
			...
			System.out.println("서버시작...");
			 ...
			 while(true) {
				 ...
				 PrintWriter pw = new PrintWriter(os,true); // 데이터를 출력할 때 println(), print() 메소드를 사용
				 ...
				 while(true) {
					 ...
					 if(message == null || "exit".equals(message)) {
						 ...
						 pw.flush(); //현재 스트림에 있는 데이터를 강제로 내보내는 메소드. close() 메소드 호출시 자동 flush()호출
					 }
				 }
