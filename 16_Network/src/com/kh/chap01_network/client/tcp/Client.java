...
public class Client {
	...
	public void clientStrat(){
		//1) 서버의 IP주소와 서버가 정한 "포트번호"를 매개변수로 하여 "클라이언트용 소켓" 객체생성
		int port = 30027; // 주소 맞춰 줘야함.
		String str = null;
		...
		try {
			...
			//서버에 연결을 요청하는 구문(요청하고자 하는 서버의 IP주소 + port번호를 제시하면서 소켓 생성)
			socket = new Socket(serverIP,port);
			
			//서버와 연결이 실패한다면 null 반환 할 것. -> 즉, null 이면 실패
			...
			if(socket != null) {
				...
				do {
					...
					System.out.println("받은 메세지는 : "+message);
				} while(true);
